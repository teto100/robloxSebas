-- SCRIPT PRINCIPAL DEL JUEGO DE CASTILLO
...
Players.PlayerAdded:Connect(function(jugador)
	jugador.CharacterAdded:Connect(function(personaje)
		...
		titulo.Text = "⚔️ BIENVENIDO AL CASTILLO ⚔️"
		...
		pregunta.Text = "¿Cuál es tu nombre?"
		...
		inputNombre.Text = ""
		inputNombre.PlaceholderText = "Escribe tu nombre aquí..."
		...
		local function aplicarSkin(clase)
			...
			print("[DEBUG] Longitud del texto:", #inputNombre.Text)
			
			local nombre = inputNombre.Text:gsub("^%s*(.-)%s*$", "%1")
			print("[DEBUG] Nombre despues de limpiar:", nombre)
			print("[DEBUG] Longitud despues de limpiar:", #nombre)
			
			if nombre == "" or #nombre < 1 then
				print("[ERROR] Nombre vacio o muy corto")
				mensajeError.Text = "Escribe un nombre valido"
				return
			end
			
			print("[OK] Nombre valido:", nombre)
			...
			elseif clase == "Luchador" then
				...
				local weld = Instance.new("Weld")
				weld.Part0 = brazoDer or personaje:FindFirstChild("RightHand")
				weld.Part1 = espada
				...
				print("Luchador creado:", nombre)
			end
			
			print("[OK] Cerrando menu")
			screenGui:Destroy()
		end
		
		print("[INIT] Conectando botones...")
		
		botonMago.MouseButton1Click:Connect(function()
			print("[CLICK] Boton Mago presionado")
			aplicarSkin("Mago")
		end)
		
		botonLuchador.MouseButton1Click:Connect(function()
			print("[CLICK] Boton Luchador presionado")
			aplicarSkin("Luchador")
		end)
		
		print("[INIT] Menu creado para:", jugador.Name)
	end)
end)
